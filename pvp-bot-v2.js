...
function pvpBotV2(bot) {
  ...
  bot.on("entitySpawn", (entity) => {
    /* Save the opponent entity as soon as it spawns, and start following it, if it is supposed to
    fight it */
  ...
  bot.on("physicsTick", () => {
    /* Don't run if the bot is not supposed to fight or if the opponent didn't spawn or load. */
  ...
  bot.on("death", () => {
    /* Stop following opponent as soon as you die to prevent high CPU usage bug. */
    stopFollowing();
  });
}
